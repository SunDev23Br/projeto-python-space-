...
def adicionar_detritos_usuario(detritos):

    print("\n=== CADASTRO DE NOVO DETRITO ===")

    while True:

        resposta = input("\nDeseja adicionar um novo detrito? (s/n): ").lower()

        if resposta != "s":
            break

        id_detrito = input("Nome/ID do detrito: ")

        x = float(input("Coordenada X: "))
        y = float(input("Coordenada Y: "))
        z = float(input("Coordenada Z: "))

        velocidade = float(input("Velocidade do detrito: "))

        novo_detrito = {
            "id_detrito": id_detrito,
            "coordenadas_xyz": [x, y, z],
            "velocidade": velocidade,
        }

        detritos.append(novo_detrito)

        print("\nDetrito adicionado com sucesso!")
...
def exibir_relatorio(resultados):
    ...
    for detrito in resultados:
        ...
        if detrito['risco'] == "CRÍTICO":
            print("⚠ ALERTA: Risco de colisão iminente!")
...
def main():
    ...
    detritos = adicionar_detritos_usuario(detritos)
